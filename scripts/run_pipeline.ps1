...
& $Python (Join-Path $Root "sarva_training\curate.py")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "`n[2/4] build_dataset.py" -ForegroundColor Yellow
& $Python (Join-Path $Root "sarva_training\build_dataset.py")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "`n[3/4] research_report.py" -ForegroundColor Yellow
& $Python (Join-Path $Root "sarva_training\research_report.py")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "`n[4/4] scheduler.py" -ForegroundColor Yellow
& $Python (Join-Path $Root "sarva_training\scheduler.py")
exit $LASTEXITCODE
