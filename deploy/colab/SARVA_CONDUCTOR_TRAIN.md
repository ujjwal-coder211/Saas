# Sarva Conductor — Colab Training (Option A bootstrap + Option B full)

Upload `colab_export.zip` from `Saas/sarva_training/data/export/` to Colab.
...
# model.load_adapter("Ujjwal211/aitotech-sarva-v1")
```
...
trainer = SFTTrainer(
    ...
    args=SFTConfig(
        output_dir="sarva_conductor_lora",
...
```python
ADAPTER_REPO = "Ujjwal211/aitotech-sarva-v2"  # bump to v3 for Round 1B

model.save_pretrained("sarva_conductor_lora")
tokenizer.save_pretrained("sarva_conductor_lora")
...
# Deploy on RunPod, set SARVA_INFERENCE_URL, then:
