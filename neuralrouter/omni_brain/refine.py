"""Omni refinement layer (paper v4 §3.4).

Ported from the saira_harvest Phase-1 `omni/refine.py`. When Omni delegates, it
treats the teacher's answer as a DRAFT — it verifies the output before returning
it, rather than forwarding blindly.

Honesty note (same as source): true refinement needs Omni's own model judgment
