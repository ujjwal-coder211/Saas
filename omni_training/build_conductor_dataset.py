...
from omni_training.conductor_schema import ROUTING_SYSTEM, SYNTHESIS_SYSTEM
from omni_training.ingest_post_train import load_jsonl, _is_routing_row, _is_synthesis_row
...
def sarva_to_cot(row: dict) -> dict | None:
    ...
    return {
        "messages": [
            {"role": "system", "content": f"You are Omni. Respond clearly in {lang}. Show reasoning then answer."},
...
def load_sarva_rows() -> list[dict]:
    paths = [
        SARVA_DIR / "output" / "omni_v1_train.jsonl",
...
def build(*, bootstrap_only: bool = False) -> dict:
    ...
    if bootstrap.exists():
        ...
        combined = SEED_DIR / "omni_combined_train.jsonl"
        if combined.exists():
