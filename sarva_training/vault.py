...
- Optional AES encryption at rest (set SARVA_VAULT_ENCRYPTION_KEY)
...
TRAIN_OUTPUT_PATH = VAULT_DIR / "sarva_v1_train.jsonl"
RESEARCH_OUTPUT_PATH = VAULT_DIR / "sarva_v1_research.jsonl"
FEEDBACK_PATH = VAULT_DIR / "feedback_patches.jsonl"

# Legacy paths (migrate reads)
LEGACY_RAW = DATA_DIR / "interactions_raw.jsonl"

VAULT_HMAC_KEY = os.environ.get("SARVA_VAULT_HMAC_KEY", "")
VAULT_ENCRYPTION_KEY = os.environ.get("SARVA_VAULT_ENCRYPTION_KEY", "")
...
def encrypt_payload(plain: str) -> str:
    ...
    try:
        ...
        f = Fernet(base64.urlsafe_b64encode(_derive_key(VAULT_ENCRYPTION_KEY, b"sarva-vault")[:32]))
...
def decrypt_payload(stored: str) -> str:
    if not stored.startswith("enc:"):
        return stored
    if not VAULT_ENCRYPTION_KEY:
        raise RuntimeError("Encrypted vault row but SARVA_VAULT_ENCRYPTION_KEY not set")
    from cryptography.fernet import Fernet
...
def verify_admin_key(provided: str | None) -> bool:
    if not SARVA_ADMIN_KEY:
        return False
    if not provided:
        return False
    return hmac.compare_digest(provided, SARVA_ADMIN_KEY)
